...
import { defaultPreviewMeter } from '../core/presentation.js';
import { effectiveQuery, cliProvider, supportsCli, arkProducts, type QueryConfig } from '../core/channel.js';
import { arkQuotaCommand, queryDescription, meterDescription, httpCurl } from '../core/methods.js';
...
export function Channels({actions,onSaved}: {actions:ChannelActions;onSaved:()=>void}) {
  ...
  const methodUnchanged=JSON.stringify([row?.query,row?.region,row?.planType,row?.arkProfile,row?.arkProduct])===JSON.stringify([saved?.query,saved?.region,saved?.planType,saved?.arkProfile,saved?.arkProduct]);
  const edit=(value:Partial<Channel>)=>{setDraft({...draft,[provider]:rows.map(r=>r.id===selected?{...r,...value}:r)});setDirty(true);setMessage('');if(value.query || value.region || value.planType || 'arkProfile' in value || 'arkProduct' in value){setCatalog(old=>{const next={...old};delete next[key];return next;});}};
  ...
  const valueText=(id:string)=>{const m=meters.find(m=>m.id===id);return m?.entitlement==='unlimited'?'无限':m?.entitlement==='unsupported'?'不在当前套餐中':m?.remainingPercent!==undefined?`${m.remainingPercent.toLocaleString(undefined,{maximumFractionDigits:1})}% 剩余`:m?.remaining!==undefined?`${m.remaining.toLocaleString()} ${m.currency ?? ''} 剩余`:'暂未返回';};
  ...
  return <div className="aim-config">
    ...
    {row && <fieldset className="aim-config-body" disabled={busy || !data?.writable}>
      ...
      {step===1 && <>
        ...
        {provider==='volcengine' && <div className="aim-connection-note"><strong>通过火山方舟官方 arkcli 查询</strong><p>在执行机器安装 npm i -g @volcengine/ark-cli，并运行 arkcli auth login 完成登录。使用已有身份查询套餐，不需要在此填写 Key。</p><label>ARK profile（可选）<input aria-label="ARK profile" value={row.arkProfile ?? ''} onChange={e=>edit({arkProfile:e.target.value || undefined})} placeholder="留空沿用 CLI 当前 profile"/></label><label>套餐类型<select aria-label="ARK 套餐类型" value={row.arkProduct ?? ''} onChange={e=>edit({arkProduct:e.target.value as Channel['arkProduct'] || undefined})}><option value="">自动发现已订阅套餐</option>{arkProducts.map(product=><option key={product} value={product}>{product}</option>)}</select></label><p>显式指定 profile 只作用于本次查询，不改变 CLI 默认配置；区域和身份由 profile 决定。指定套餐可减少探测请求，团队套餐查询当前用户绑定的席位。</p><div className="aim-curl"><pre><code>{arkQuotaCommand(row)}</code></pre></div><p>查询套餐周期额度（session / 5h / weekly / monthly），不包含按量 API 账单或模型明细；Agent Plan 数量单位是 AFP。查询失败不会自动启动登录或切换身份。</p><a href="https://docs.volcengine.com/docs/82379/2536875?lang=zh" target="_blank" rel="noreferrer">官方 CLI 说明 ↗</a></div>}
        {provider==='minimax' && q?.kind==='cli' && <div className="aim-connection-note"><strong>通过 MiniMax 官方 mmx CLI 查询</strong><p>在执行机器安装 mmx-cli，并运行 mmx auth login 完成登录；本机和 SSH 都使用该机器上 mmx 的登录、区域及套餐配置，下面无需填写 Key。</p><div className="aim-curl"><pre><code>mmx quota show --non-interactive --quiet --output json</code></pre></div><p>当前解析 Token Plan 的 model_remains；按量 API 余额返回暂不支持。可用 mmx quota show 对照官方文本显示。</p><a href="https://github.com/MiniMax-AI/cli" target="_blank" rel="noreferrer">官方安装与登录说明 ↗</a></div>}
        ...
        {provider==='minimax' && q?.kind==='http' && <><label>区域<select value={row.region ?? 'global'} onChange={e=>edit({region:e.target.value as 'global'|'cn'})}><option value="global">国际站</option><option value="cn">国内站</option></select></label><label>套餐<select value={row.planType ?? 'token'} onChange={e=>edit({planType:e.target.value as 'token'|'coding'})}><option value="token">Token Plan</option><option value="coding">旧 Coding Plan</option></select></label></>}
        <details><summary>刷新周期</summary><label>缓存与自动检查间隔<select aria-label="刷新周期" value={row.refreshIntervalSeconds ?? ''} onChange={e=>edit({refreshIntervalSeconds:e.target.value?Number(e.target.value):undefined})}><option value="">自动（HTTP 2 分钟 / CLI、SSH 5 分钟）</option>{[30,60,120,300,600,1800,3600].map(n=><option key={n} value={n}>{n<60?`${n} 秒`:`${n/60} 分钟`}</option>)}</select></label><p className="aim-config-help">仅在查看用量时检查；关闭用量界面不后台轮询。查询失败会延长重试间隔，手动校验可立即重试。</p></details>
