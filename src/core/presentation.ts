...
import type { Overview, ProviderId, QuotaMeter } from './types.js';
import { overview } from './normalize.js';
export type AccountMap=Partial<Record<ProviderId,AccountConfig[]>>;
export const defaultPreviewMeter=(m:QuotaMeter)=>m.kind!=='credits' && m.entitlement!=='unsupported';
...
export function projectPreview(value:Overview,accounts:AccountMap):Overview{
  const rows=value.snapshots.filter(s=>previewIncluded(accounts,s.provider,s.account.id)).map(s=>{
    ...
    const meters=selected?selected.flatMap(id=>{const m=s.meters.find(m=>m.id===id);return m?[m]:[];}):s.meters.filter(defaultPreviewMeter);
    const missingMeters=selected?.filter(id=>!s.meters.some(m=>m.id===id)).map(id=>labels[id] || id);
    return {...s,assessmentMeters:s.assessmentMeters ?? s.meters,meters:meters.map(m=>({...m,name:labels[m.id] || m.name})),...(missingMeters?.length?{missingMeters}:{}),order:p?.order ?? 0};
